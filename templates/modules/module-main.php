...
<section class="module main full-size container-fluid <?=$GLOBALS['device'];?>">
  ...
  <?php if ($module_background) : $background_full = wp_get_attachment_image_src($module_background, '1360x768'); $background_min = wp_get_attachment_image_src($module_background, 'medium'); ?>
    <div class="background async-bg <?=$GLOBALS['device'];?>" data-background="<?=$background_full[0];?>" data-speed="2">
      <div class="temp-background" style="background-image: url(<?=$background_min[0];?>);"></div>
    </div>
  <?php endif; ?>
